...
export default function ContactClient({ settings }: { settings: ContactSettings }) {
    ...
    return (
        <div ref={mainRef} className="min-h-screen bg-white text-[#1a1a1a] font-sans selection:bg-[#FF2800] selection:text-white overflow-x-hidden">
            <header className="relative pt-32 md:pt-40 pb-20 md:pb-28 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto min-h-[70vh] flex flex-col justify-end">
                <div ref={bigTextRef} className="absolute top-0 left-0 w-full pointer-events-none -z-10 overflow-hidden select-none">
                    <h2 className="text-[22vw] leading-[0.8] serif-font font-bold text-[#1a1a1a] opacity-[0.03] translate-y-[10%]">CONTACT</h2>
                </div>
                <div ref={titleRef}>
                    <div className="overflow-hidden mb-1"><span className="serif-font italic text-sm text-[#FF2800] block">{settings.availability_label}</span></div>
                    <div className="overflow-hidden"><h1 className="text-7xl md:text-8xl lg:text-9xl serif-font leading-[0.9] tracking-tight">Let&apos;s craft</h1></div>
                    <div className="overflow-hidden"><h1 className="text-7xl md:text-8xl lg:text-9xl serif-font leading-[0.9] tracking-tight"><span className="italic">something</span> <span className="text-[#FF2800]">rare.</span></h1></div>
                    <div className="overflow-hidden mt-8 md:mt-12"><p className="text-gray-500 text-lg md:text-xl font-light max-w-lg leading-relaxed">Have a project in mind? Drop me a line and let&apos;s build something <span className="text-[#1a1a1a] font-normal border-b-2 border-[#FF2800]">unforgettable</span>.</p></div>
                </div>
            </header>
                    ...
                    <div ref={infoRef} className="order-2 lg:order-1 lg:col-span-5 space-y-16">
                        <div className="reveal-section">
                            ...
                            <div className="flex items-center gap-6 mt-6">
                                ...
                                <div><span className="text-2xl serif-font">{settings.location}</span><p className="text-sm text-gray-400">{settings.location_note}</p></div>
                            </div>
                        </div>
                        <div className="reveal-section">
                            ...
                            <div className="space-y-0">
                                {settings.social_links.map((link) => (
                                    <a key={link.name} href={link.url} target={link.url.startsWith('http') ? '_blank' : undefined} rel={link.url.startsWith('http') ? 'noopener noreferrer' : undefined} className="social-link-item flex items-center justify-between py-5 border-b border-gray-100 group cursor-pointer">
                                        <span className="font-medium tracking-wide text-lg">{link.name}</span>
                                        <svg className="w-4 h-4 text-[#FF2800] opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                                    </a>
                                ))}
                            </div>
                        </div>
                        <div className="reveal-section hidden lg:block">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-xs font-semibold tracking-wide uppercase text-gray-900">
                                {settings.skills.map((item) => <div key={item} className="hover:text-[#FF2800] transition-colors cursor-default">{item}</div>)}
                            </div>
                        </div>
                    </div>
                        ...
                        <div className="bg-[#fafafa] border border-gray-100 p-8 md:p-12 relative overflow-hidden">
                            {status === 'success' && (
                                <div className="success-overlay absolute inset-0 bg-white z-20 flex flex-col items-center justify-center text-center p-8">
                                    <div className="mb-6"><svg className="w-16 h-16 text-[#FF2800]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg></div>
                            ...
                            {status === 'error' && <div className="mb-6 p-4 bg-red-50 border border-red-100 text-red-600 text-sm">{errorMsg}</div>}
                            <form onSubmit={handleSubmit} className="space-y-10 relative z-10">
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {inquiryTypes.map((type) => (
                                            <button key={type.value} type="button" onClick={() => setActiveInquiry(activeInquiry === type.value ? '' : type.value)} className={`px-5 py-2.5 text-xs font-bold tracking-[0.05em] uppercase transition-all duration-300 border ${activeInquiry === type.value ? 'bg-[#FF2800] text-white border-[#FF2800]' : 'bg-transparent text-gray-600 border-gray-200 hover:border-[#FF2800] hover:text-[#FF2800]'}`}>{type.label}</button>
                                        ))}
                                    </div>
                                </div>
                                <div className="relative group">
                                ...
                                <button type="submit" disabled={status === 'submitting'} className="group relative inline-flex items-center h-14 overflow-hidden disabled:opacity-60 disabled:cursor-not-allowed">
                                    <span className="absolute inset-0 bg-[#FF2800]" /><span className="absolute inset-0 bg-[#1a1a1a] origin-bottom scale-y-0 group-hover:scale-y-100 transition-transform duration-500 ease-[cubic-bezier(0.65,0,0.35,1)]" /><span className="relative z-10 text-white text-xs font-bold tracking-widest uppercase pl-8 pr-2 group-hover:pl-7 transition-all duration-500 ease-out">{status === 'submitting' ? 'Sending...' : 'Send Inquiry'}</span>
                                    <span className="relative z-10 flex items-center justify-center w-10 h-10 mr-2 bg-white/20 group-hover:bg-white transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:scale-110">
                                        {status === 'submitting' ? <svg className="w-4 h-4 text-white group-hover:text-[#1a1a1a] animate-spin transition-colors duration-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg> : <svg className="w-4 h-4 text-white group-hover:text-[#1a1a1a] transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] group-hover:translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M4 12h16m0 0l-6-6m6 6l-6 6" /></svg>}
                                    </span>
                                </button>
                            </form>
                        </div>
            ...
            <section className="py-20 md:py-28 px-6 md:px-12 lg:px-24 max-w-7xl mx-auto text-center">
                ...
                <a href={`mailto:${settings.email}`} className="group inline-block serif-font italic text-3xl sm:text-4xl md:text-6xl text-[#1a1a1a] hover:text-[#FF2800] transition-colors duration-300 relative">{settings.email}<span className="absolute left-0 -bottom-2 w-0 group-hover:w-full h-[2px] bg-[#FF2800] transition-all duration-500" /></a>
            </section>
            <Footer />
        </div>
    );
